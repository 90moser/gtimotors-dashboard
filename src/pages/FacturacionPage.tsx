...
const FacturacionPage = () => {
  ...
  const handleBuscarClienteMensual = async () => {
    ...
    try {
      ...
      // Load 'listo' citas not yet invoiced
  ...
  // IVA: prices already include VAT
    ...
    <PageLayout title="Facturación">
      ...
      <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
        ...
        <div className="flex gap-2">
          <Button
            variant="outline"
            onClick={() => setDialogMensualOpen(true)}
            className="gap-2"
          >
      ...
      <Dialog open={dialogMensualOpen} onOpenChange={handleDialogMensualOpenChange}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] flex flex-col p-0 gap-0">
                ...
                {citasMensual.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      >
                        {seleccionadas.size === citasMensual.length
                          ? "Deseleccionar todas"
                          : "Seleccionar todas"}
                      </button>
                    </div>
                    ...
                    {seleccionadas.size > 0 && (
                      <div className="rounded-xl border border-border bg-muted/40 p-4 space-y-2">
                        ...
                        <div className="flex justify-between text-sm">
                          <span className="text-muted-foreground">IVA (21%)</span>
                          <span>
                            {(totalMensual - totalMensual / 1.21).toFixed(2)} €
                          </span>
                        </div>
                        <div className="flex justify-between text-sm font-semibold border-t border-border pt-2">
                          <span>Total</span>
          ...
          <div className="flex-shrink-0 border-t border-border px-6 py-4 flex justify-end gap-3 bg-background">
            <Button
              variant="outline"
              onClick={() => handleDialogMensualOpenChange(false)}
              disabled={creandoMensual}
            >
              Cancelar
            </Button>
            <Button
            ...
            >
              {creandoMensual ? (
                ...
                <>
                  <Download className="h-4 w-4" />
                  Crear Factura Mensual y Descargar PDF
                </>
              )}
            </Button>
          </div>
